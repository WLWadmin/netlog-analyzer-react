...
/**
 * 获取某个 source 的完整依赖链（向下查找被依赖的）
 */
export function getChainForSource(graph: SourceGraph, sourceId: number): SourceNode[] {
  const chain: SourceNode[] = [];
  const visited = new Set<number>();

  // 先向上找到根（URL_REQUEST）
  let current = sourceId;
  const upPath: number[] = [current];
  visited.add(current);
