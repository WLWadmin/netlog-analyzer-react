  ...
  return (
    ...
    >
      ...
      <div style={{ marginTop: 8, marginBottom: 8 }}>
        <Tag style={{ margin: 0, border: 'none', borderRadius: 999, color, background: `${color}14`, fontWeight: 700 }}>
          {value}
        </Tag>
      </div>
      <Typography.Text style={{ fontSize: 13, lineHeight: 1.7, color: 'var(--text-secondary)' }}>
        {detail}
      </Typography.Text>
    </div>
  );
...
const NetlogMetricExplainPanel: React.FC<NetlogMetricExplainPanelProps> = ({ result }) => {
  ...
  return (
    <Card title="核心指标解释" style={{ background: 'var(--bg-elevated)', borderColor: 'var(--border-color)' }} styles={{ body: { padding: 16 } }}>
